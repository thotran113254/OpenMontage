...
### Talking-head footage → use the auto-edit job runner

When the user hands over **raw talking-head footage** and wants it cut into a short-form video
(captions, cards, punch-ins, cold-open), do NOT improvise ffmpeg/Remotion calls and do NOT use the
seconds-based `footage_edit_analyzer`. Run the word-anchored job pipeline:

```bash
# one file
python -m lib.talking_head_edit.cli --input <file> --topic "..." --prompt "<yêu cầu riêng>"

# several takes of the same content, or a folder — the pipeline picks the best take
...
# run it unattended: fixes one KNOWN failure and reports honestly; unknown failures stop
python -m lib.talking_head_edit.cli --job <job_id> --autopilot
```

It owns transcription, take selection, the AI director, a cut audit, cutting, rendering and
post-render verification, and it is resumable per stage (`--stage-from`, `--stages`, `--revise`,
`--chat`). Full contract, in two files: [`docs/talking-head-autoedit.md`](docs/talking-head-autoedit.md)
for one build end to end, and
[`docs/talking-head-autoedit-platform.md`](docs/talking-head-autoedit-platform.md) for the
many-sources / many-builds layer (ASR providers, projects, prompt registry). A local
web UI over the same pipeline runs with `make autoedit-server` + `make autoedit-ui`.

Footage a user will build from more than once belongs in a **project** — sources are uploaded once and
shared by every build, with the assembly config and keyterms attached to the shoot rather than to one
job:

```bash
python -m lib.talking_head_edit.cli --projects                       # list
python -m lib.talking_head_edit.cli --project <id> --prompt "..."    # build inside one
```

ASR defaults to ElevenLabs Scribe (one API call, ~10-30 s) and falls back to local Whisper with a
warning when `ELEVENLABS_API_KEY` is missing. Prompts are versioned data under `prompts/`, editable
and A/B-testable from the UI — do NOT hardcode prompt text when changing director behaviour.
...
### Render Location — Local vs Cloud

Orthogonal to *runtime* (Remotion/HyperFrames/FFmpeg) and *authoring mode* (templated/atelier) is
*location*: **where** the render actually runs. Local vs a rented Vast.ai box is an asymmetric
decision, unlike runtime and authoring mode:

- **Local is a legitimate silent default** — it costs nothing and is the status quo. No
  announcement is required to render locally.
- **Cloud is never silent.** Renting spends real money and moves footage off this machine, so it
  requires the same 3-part presentation + explicit approval as the runtime HARD RULE above,
  **every single rental** — there is no standing pre-approval and no unattended/overnight
  auto-flush, ever. See `skills/core/cloud-render.md` for the mandatory announce template, the
  crossover numbers, and the batch policy. For the human-facing setup/CLI/troubleshooting guide,
  see `docs/cloud-render.md`.

Log every cloud decision — sample or batch — as `category: "render_location_selection"` in
`decision_log`, with **both** `local` and `cloud` in `options_considered`, same standard as
`render_runtime_selection`. If cloud becomes unreachable or exceeds a ceiling after being
recommended, escalate per "Escalate Blockers Explicitly" below and wait — do not silently fall back
to local, and do not fall forward to cloud just because local looks slow.
...
- **object_storage** — Durable off-machine project storage (`cloudflare_r2`). Opt-in — ships
  disabled; read the tool's `install_instructions` from the registry rather than hardcoding env
  var names here. See `docs/r2-storage.md` for the human-facing setup guide.
