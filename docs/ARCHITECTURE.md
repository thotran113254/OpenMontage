...
│   ├── cloud_render/       # Vast.ai rented-box render: config, vast_client, ledger+reaper,
│   │                       # onstart/transfer, kit packer, remote render, batch queue
│   │                       # (human entry point: docs/cloud-render.md)
...
│                           # incl. vast_cloud_render.py + cloud_render_queue.py (capability="cloud_render")
│
...
**Analysis (6):** transcriber (WhisperX), scene_detect, frame_sampler, video_understand (CLIP/BLIP-2), footage_edit_analyzer (Gemini), speech_gap_detector
...
**Graphics (14):** flux_image, grok_image, google_imagen, openai_image, recraft_image, local_diffusion, pexels_image, pixabay_image, image_selector, code_snippet, diagram_gen, math_animate (ManimCE), nine_router_image (NineRouter provider), image_gen (deprecated)
...
| `GEMINI_API_KEY` | footage_edit_analyzer, speech_gap_detector | Google Gemini API for video analysis and speech detection |
...
- `ugc-talking-head.yaml` — UGC editing style with zoom rhythm, transitions, and filler-word removal for raw talking-head footage
