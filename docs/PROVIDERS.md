...
GEMINI_API_KEY=              # Google Gemini API for video analysis and speech detection
...
# CLOUD RENDER (rents a short-lived CPU box for composition rendering; disabled by default)
VAST_API_KEY=                # Vast.ai — cloud_render capability, must also flip
                              # config/cloud-render.json enabled:true
...
### Google — TTS + Imagen + Gemini

> **Multi-tool coverage.** Google Cloud TTS has 700+ voices in 50+ languages — the strongest localization option. Imagen 4 generates high-quality images. Gemini API powers video analysis and speech processing.

**Tools unlocked:** `google_tts`, `google_imagen`, `footage_edit_analyzer`, `speech_gap_detector`
**Env vars:** `GOOGLE_API_KEY` (for TTS + Imagen), `GEMINI_API_KEY` (for Gemini video/speech tools)
...
**For Gemini API** (video analysis, speech processing):
1. You can use the same `GOOGLE_API_KEY` or create a separate `GEMINI_API_KEY` by repeating steps 1-4 above
2. Add to `.env`: `GEMINI_API_KEY=AIza...` (or omit if using the same key as `GOOGLE_API_KEY`)
3. The Generative Language API (Gemini) must be enabled in your Google Cloud project — visit [console.cloud.google.com/apis/library/generativelanguage.googleapis.com](https://console.cloud.google.com/apis/library/generativelanguage.googleapis.com) and ensure it's enabled
...
**New account bonus:** Google Cloud offers **$300 in free credits** for new accounts (90-day trial), applicable to both TTS, Imagen, and Gemini.

#### Google Gemini API Pricing

| Model | Input (per 1M tokens) | Output (per 1M tokens) | Use Case |
|-------|----------------------|------------------------|----------|
| Gemini 2.0 Flash | $0.075 | $0.30 | Fast, cost-effective video/speech analysis |
| Gemini 2.0 Flash with Thinking | $0.30 | $1.20 | Complex reasoning and detailed analysis |

**Free tier:** 15 requests per minute (quota-based, not time-based), 2,000 requests per day across all free-tier APIs.

**Best for:** Video understanding, frame analysis, transcript annotation. Token pricing is based on input/output volume, not duration.
...
### Vast.ai — Cloud Render (Rented CPU Box)

> **Rent a short-lived box to render a composition off your own machine.** Not a generation
> provider — this unlocks the `cloud_render` capability (`vast_cloud_render` /
> `cloud_render_queue`), which uploads a render kit, renders it on a rented box, downloads the
> output, and destroys the instance. Worth it for long renders or a queued batch; overhead-heavy
> for a single short job.

**Tools unlocked:** `vast_cloud_render`, `cloud_render_queue`
**Env var:** `VAST_API_KEY` (or the `vastai` CLI's own config file at
`~/.config/vastai/vast_api_key`)

#### Setup

1. Create a Vast.ai account and generate an API key.
2. `pip install vastai`.
3. Generate the dedicated cloud-render SSH keypair (not a shared/reused key):
   `python -m lib.cloud_render.setup_key` → writes `~/.ssh/openmontage_cloud_render`.
4. Add to `.env`: `VAST_API_KEY=...`.
5. Explicitly enable it in `config/cloud-render.json` — **it ships with `"enabled": false`** and
   stays off until a human flips this; there is no auto-enable on first successful rental.

#### What it's best for

- Renders long enough that ~6 minutes of paid overhead (boot + `npm ci` + teardown) is small
  relative to the render itself — the agent's crossover rule is roughly "local render time > 2×
  overhead, or ≥3 jobs already queued."
- Freeing this machine during a long render or a batch of queued jobs.
- **Not** worth it for a single short render, and not appropriate for confidential footage — the
  data leaves this machine for a third-party box for the duration of the rental. See
  `skills/core/cloud-render.md` for the full decision policy; every rental requires an explicit
  announce and human approval, with no unattended/overnight auto-flush.

#### Pricing (measured, 32-vCPU offer, verified 2026-08-06)

| Pricing mode | Price | Notes |
|---|---|---|
| **Bid (interruptible) — default** | ~$0.069/hr | ~15% cheaper than on-demand; can be preempted mid-render, which this system treats as the normal case (requeue + re-announce + retry), not an edge case |
| On-demand (non-interruptible) | ~$0.0814/hr | Opt-in when a mid-render restart would be too costly to risk |

Ceilings enforced by config (fail loud, never silently widened): `max_dph_usd: 0.15`,
`max_total_usd_per_rental: 0.50`, `max_runtime_minutes: 60`.

**Free tier:** None — pay-as-you-go per rental-hour. No cost until a rental is explicitly approved.

---

### Cloudflare R2 — Object Storage

> **Durable off-machine storage for project assets, and the transfer medium for cloud render.**
> Not a generation provider — this unlocks the `object_storage` capability (`cloudflare_r2`),
> which syncs `projects/**` job/project directories to R2 and (when cloud render is also enabled)
> carries the Vast.ai kit upload/result download via presigned URLs instead of `scp`.

**Tools unlocked:** `cloudflare_r2`
**Env vars:** `CLOUDFLARE_R2_ACCOUNT_ID`, `CLOUDFLARE_R2_ACCESS_KEY_ID`,
`CLOUDFLARE_R2_SECRET_ACCESS_KEY`, `CLOUDFLARE_R2_ENDPOINT_URL`, `CLOUDFLARE_R2_BUCKET`,
`CLOUDFLARE_R2_PUBLIC_BASE_URL` (optional — enables public delivery links)

#### Setup

See `docs/r2-storage.md` for the full walkthrough, including the two manual Cloudflare-dashboard
steps (bucket creation; optional public access/custom domain) that no script here can do for you.
Short version:
...
3. Fill in the `CLOUDFLARE_R2_*` vars in `.env` (see `.env.example`).
4. Flip `"enabled": true` in `config/r2-storage.json` — **it ships with `"enabled": false"`**
   and stays off until a human flips this.

#### What it's best for

- Archiving finished jobs/projects off-machine without a human remembering to run a backup.
- Sharing a finished render via a link (`delivery_url` — public once a custom domain is
  configured, else a presigned URL with a warning).
- The Vast.ai cloud-render transfer path (`docs/cloud-render.md`) — the rented box pulls its kit
  from R2 and pushes its output back, both via short-lived presigned URLs, and never holds a
  long-lived credential.
- **Not** a CDN tier and not multi-tenant auth — see `docs/r2-storage.md`'s non-goals.
...
| Egress | **Free** (the reason R2 was chosen over S3 for this) |

**Free tier:** 10 GB storage / month on Cloudflare's free plan; pay-as-you-go beyond that. No cost
until `enabled: true` is set and something actually syncs.
...
| **Google Gemini** | `GEMINI_API_KEY` | `footage_edit_analyzer`, `speech_gap_detector` | Free tier + paid |
...
| **Vast.ai** | `VAST_API_KEY` | `vast_cloud_render`, `cloud_render_queue` | Pay-as-you-go per rental-hour (disabled by default) |
