...
interface ProgressBarProps {
  ...
  // When true, anchors the bar to a lower-third pill with a transparent
  // root instead of painting a full-bleed opaque background. Used when the
  // bar sits over a backgroundVideo (anchor footage must stay visible —
  // an opaque AbsoluteFill here previously blacked out the whole scene).
  lowerThird?: boolean;
}
...
export const ProgressBar: React.FC<ProgressBarProps> = ({
  ...
  // Lower-third variant: light text over a dark pill, never an opaque
  // full-bleed background (the anchor footage behind must stay visible).
  const effectiveTextColor = lowerThird ? "#F8FAFC" : textColor;
