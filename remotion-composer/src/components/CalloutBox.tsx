...
interface CalloutBoxProps {
  ...
  // Anchors the box to a lower-third banner instead of dead center, so it
  // doesn't cover the speaker's face/mouth when shown over a still-playing
  // backgroundVideo. See TextCard's matching prop / SceneRenderer.
  lowerThird?: boolean;
}
...
export const CalloutBox: React.FC<CalloutBoxProps> = ({
  ...
  // Icon entrance — slightly delayed. Kept short (2 frames, not the
  // previous 5) so the box's own opacity spring (above) can't finish
  // rising to "visually solid" before the icon starts appearing — a wider
  // gap here reads as a blank placeholder box flashing before content
  // shows up (confirmed by frame-by-frame inspection of a real render).
  ...
  // Text fade in — staggered after box, but tightened (4 frames, not 8)
  // for the same blank-flash reason as the icon above.
  ...
  return (
    <AbsoluteFill
      style={{
        background: lowerThird ? "transparent" : containerBackgroundColor,
        justifyContent: lowerThird ? "flex-end" : "center",
        alignItems: "center",
        paddingBottom: lowerThird ? "10%" : 0,
      }}
    >
      <div
        style={{
