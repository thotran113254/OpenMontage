...
import { resolveCutTransition } from "./transitions/preset-map";
import { buildRenderGroups } from "./transitions/build-render-groups";
...
interface Cut {
  ...
  // Duration override (seconds) for the transition_in preset, per the
  // edit_decisions schema. Falls back to a short fixed default when unset.
  transition_duration?: number;
  transform?: {
    animation?: string;
    scale?: number;
    // Animated zoom endpoints for video cuts (primary VideoScene or the
    // backgroundVideo behind an overlay). Used when animation is
    // "zoom_in"/"zoom_out"; ignored otherwise. `scale` remains the static,
    // non-animated punch-in used to mask a ripple-delete splice point.
    scaleFrom?: number;
    scaleTo?: number;
    position?: string | { x: number; y: number };
  };
  // Continuation flag: this cut renders the SAME editorial card as the
  // previous cut (a single card split across a pause-tighten splice). The
  // card must not replay its entrance animation on this segment.
...
// Resolves the scale a video-backed cut should render at this frame.
// - zoom_in/zoom_out (with scaleFrom/scaleTo present): a real animated
//   interpolation across the cut's full duration — dynamic camera motion
//   driven by footage_edit_plan's beats[].zoom.action.
// - otherwise: the existing static punch-in (e.g. splice-masking) value,
//   unchanged — non-zoom cuts keep behaving exactly as before.
function resolveVideoScale(
  ...
  const isAnimatedZoom =
    (zoomAnimation === "zoom_in" || zoomAnimation === "zoom_out") &&
    scaleFrom !== undefined &&
    scaleTo !== undefined;
  if (!isAnimatedZoom) return staticScale;
  return interpolate(frame, [0, Math.max(durationInFrames, 1)], [scaleFrom!, scaleTo!], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
}
...
const VideoScene: React.FC<
  { src: string; startFrom?: number; scale?: number } & VideoZoomProps
> = ({ src, startFrom = 0, scale = 1, zoomAnimation, scaleFrom, scaleTo }) => {
  const frame = useCurrentFrame();
  const { fps, durationInFrames } = useVideoConfig();

  // No baked-in fade in/out: video cuts are hard cuts. Opacity dips at
  // every boundary read as a dark flicker on continuous footage and
  // expose exactly the splice points that punch-zoom is meant to mask.
  // Dissolves between cuts belong to TransitionSeries (transition_in).
  const currentScale = resolveVideoScale(
    frame, durationInFrames, scale, zoomAnimation, scaleFrom, scaleTo
  );
    ...
    <AbsoluteFill style={{ background: "#0F172A" }}>
      <OffthreadVideo
        ...
        style={{
          ...
          height: "100%",
          objectFit: "cover",
          // Animated zoom_in/zoom_out (footage_edit_plan-driven dynamic
          // camera motion) when set, otherwise the static punch-in used to
          // visually mask a ripple-delete splice point (e.g. a compressed
          // dead-air pause) inside an otherwise continuous talking-head
          // shot. Driven by edit_decisions cuts[].transform.
...
> = ({ src, startFrom = 0, overlayOpacity = 0.55, children, zoomAnimation, scaleFrom, scaleTo }) => {
  const frame = useCurrentFrame();
  const { fps, durationInFrames } = useVideoConfig();
  const currentScale = resolveVideoScale(frame, durationInFrames, 1, zoomAnimation, scaleFrom, scaleTo);

  return (
    <AbsoluteFill style={{ overflow: "hidden" }}>
      {/* Background video — same dynamic zoom_in/zoom_out support as the
          primary VideoScene, so b-roll cutaway beats aren't visually flat
          just because the anchor footage is dimmed/covered by a card. */}
      <OffthreadVideo
        src={resolveAsset(src)}
        startFrom={Math.round(startFrom * fps)}
        style={{
...
const SceneRenderer: React.FC<{ cut: Cut; theme: ThemeConfig }> = ({ cut, theme }) => {
    ...
    if (cut.backgroundVideo) {
      return (
        <BackgroundVideoLayer
          ...
          zoomAnimation={cut.transform?.animation}
          scaleFrom={cut.transform?.scaleFrom}
          scaleTo={cut.transform?.scaleTo}
        >
          {element}
        </BackgroundVideoLayer>
      );
  ...
  // Cutaway/overlay beats with a backgroundVideo show the anchor footage
  // (dimmed) behind the card. Anchor these to a lower-third banner instead
  // of dead center so the card doesn't sit directly over the speaker's
  // face/mouth (confirmed defect: centered cards covered the chin/mouth
  // area on real talking-head footage).
  const isFootageOverlay = Boolean(cut.backgroundVideo);

  // Explicit component types — use theme-derived defaults for colors
  if (cut.type === "text_card" && cut.text) {
    return maybeWrapWithBg(
      <TextCard
        text={cut.text} fontSize={cut.fontSize} color={textColor} backgroundColor={bgColor}
        accentColor={accent}
        lowerThird={isFootageOverlay}
        continuation={cut.cardContinuation}
      />
    );
  }
  if (cut.type === "stat_card" && cut.stat) {
    return maybeWrapWithBg(
      <StatCard
        stat={cut.stat} subtitle={cut.subtitle} accentColor={accent} backgroundColor={bgColor}
        color={textColor} boxed={isFootageOverlay}
      />
    );
  }
  ...
  if (cut.type === "progress_bar" && cut.progress !== undefined) {
    // Over anchor footage (backgroundVideo) the bar renders as a
    // lower-third pill with a transparent root — an opaque AbsoluteFill
    // here previously blacked out the entire scene (confirmed defect).
    return maybeWrapWithBg(
      <AbsoluteFill
        style={{
          background: isFootageOverlay ? "transparent" : (bgColor || theme.surfaceColor),
          display: "flex", alignItems: "center", justifyContent: "center",
          padding: isFootageOverlay ? 0 : "80px 120px",
        }}
      >
        {cut.title && !isFootageOverlay && (
          <div style={{
        ...
        <ProgressBar
          ...
          label={isFootageOverlay && cut.title
            ? `${cut.title} — ${cut.progressLabel ?? ""}`.replace(/ — $/, "")
            : cut.progressLabel}
          color={cut.progressColor || accent}
          animationStyle={(cut.progressAnimation as any) || "fill"}
          segments={cut.progressSegments}
          backgroundColor={isFootageOverlay ? "transparent" : (cut.backgroundColor || theme.surfaceColor)}
          lowerThird={isFootageOverlay}
        />
      </AbsoluteFill>
    );
...
const OverlayRenderer: React.FC<{ overlay: Overlay }> = ({ overlay }) => {
  // Each branch checks the field it renders from: an overlay whose text the
  // plan left out has nothing to show, same reasoning as provider_chip below.
...
export const Explainer: React.FC<ExplainerProps> = (props) => {
  ...
  // The animated gradient/orb background is expensive (per-frame blur) and
  // invisible whenever every cut fully covers the canvas with footage
  // (full-screen video/image or a backgroundVideo/backgroundImage layer)
  // with no timeline gaps. Skip it in that case — pure render savings.
  const backgroundCovered = (() => {
    if (cuts.length === 0) return false;
    const sorted = [...cuts].sort((a, b) => a.in_seconds - b.in_seconds);
    let coveredTo = 0;
    for (const c of sorted) {
      const covers = Boolean(
        c.backgroundVideo ||
        c.backgroundImage ||
        (c.source && !c.type && (isVideo(c.source) || isImage(c.source)))
      );
      if (!covers || c.in_seconds > coveredTo + 0.05) return false;
      coveredTo = Math.max(coveredTo, c.out_seconds);
    }
    return true;
  })();
  ...
  return (
    <AbsoluteFill style={{ background: theme.backgroundColor, fontFamily: theme.headingFont || fontFamily }}>
      ...
      {!backgroundCovered && <AnimatedBackground theme={theme} />}

      {/* Layer 1: Visual scenes — flat cuts render exactly as before (absolute
          <Sequence> positioning); only contiguous runs with an activated
          transition_in get wrapped in <TransitionSeries>. See
          transitions/build-render-groups.ts for the additive-safety rule. */}
      {buildRenderGroups(cuts, fps).map((group) => {
        if (group.kind === "flat") {
          const cut = group.cut;
        ...
        const runStart = Math.round(group.cuts[0].in_seconds * fps);
        const durations = group.cuts.map((cut) => Math.round((cut.out_seconds - cut.in_seconds) * fps));

        return (
          <TransitionSeries key={`run-${group.cuts[0].id}`} from={runStart}>
            {group.cuts.flatMap((cut, idx) => {
              ...
              elements.push(
                <TransitionSeries.Sequence key={cut.id} durationInFrames={durations[idx]}>
                  <SceneRenderer cut={cut} theme={theme} />
