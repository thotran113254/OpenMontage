...
import { MonaSample } from "./mona/MonaSample";
import { MonaTimeline } from "./mona/MonaTimeline";
import { CollageBurst, CollageBurstProps } from "./CollageBurst";
import { LyricOverlay, LyricOverlayProps } from "./LyricOverlay";
import { computeContentDurationInFrames } from "./transitions/build-render-groups";
...
const calculateMetadata: CalculateMetadataFunction<ExplainerProps> = async ({
  ...
  // End exactly where rendered content ends. TransitionSeries overlaps
  // shrink the on-screen timeline below the last cut's out_seconds, so
  // padding or out_seconds-based math leaves seconds of dead background
  // (black tail) after the content — computeContentDurationInFrames
  // mirrors the renderer's grouping + transition clamping.
  return {
    durationInFrames: Math.max(1, computeContentDurationInFrames(cuts, 30)),
  };
};
...
export const Root: React.FC = () => {
  return (
    <>
      ...
      <Composition
        id="MonaSample"
        component={MonaSample}
        durationInFrames={30 * 95}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          videoSrc: "mona_sample_src.mp4",
          lines: [],
          keywords: [],
      ...
      <Composition
        id="MonaTimeline"
        component={MonaTimeline}
        durationInFrames={30 * 120}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          videoSrc: "mona_wordspine_src.mp4",
          events: [],
          brandPill: "3 SAI LẦM • CHATBOT AI",
        }}
        // The resolver already knows the exact timeline length (cut footage +
        // cold-open teaser + endcard) and writes it into props. Deriving the
        // duration from it keeps the render from ending early or trailing
        // black frames, and removes the need to pass a --frames range.
        calculateMetadata={({ props }) => {
          const seconds = Number(props.durationSeconds) || 0;
          return seconds > 0
