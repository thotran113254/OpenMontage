...
class FaceEnhance(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "skin_smooth_strength": {
                ...
                "maximum": 1.0,
                "description": (
                    "Tunable skin-smoothing intensity, overrides preset/presets when set. "
                    "0 = barely-there smoothing, 1 = strongest before looking plastic. "
                    "Scales the same smartblur filter soft_skin uses instead of a fixed preset strength."
                ),
            },
            "custom_vf": {
                "type": "string",
    ...
    idempotency_key_fields = ["input_path", "preset", "presets", "custom_vf", "skin_smooth_strength"]
    ...
    def _build_filter(self, inputs: dict[str, Any]) -> str:
        ...
        if "skin_smooth_strength" in inputs:
            return self._smartblur_filter(inputs["skin_smooth_strength"])
    ...
    @staticmethod
    def _smartblur_filter(strength: float) -> str:
        """Map a 0-1 strength to smartblur params, spanning below and above soft_skin's fixed values."""
        strength = max(0.0, min(1.0, strength))
        lr = 0.5 + strength * 1.5
        ls = -(0.15 + strength * 0.65)
        lt = -3.0
        cr = lr * 0.5
        cs = ls
        ct = lt
        return f"smartblur=lr={lr:.2f}:ls={ls:.2f}:lt={lt}:cr={cr:.2f}:cs={cs:.2f}:ct={ct}"
