...
class VideoCompose(BaseTool):
    ...
    @staticmethod
    def _frame_mean_luma(frame_path: Path) -> float | None:
        """Mean luma (signalstats YAVG, 0-255) of one extracted frame.

        Used by the visual spotcheck to detect effectively-black frames.
        Returns None when ffmpeg/signalstats is unavailable or parsing fails.
        """
        try:
            proc = subprocess.run(
                [
    ...
    def _run_final_review(
            ...
            try:
                ...
                for i, pct in enumerate(sample_points):
                    ...
                    if frame_path.exists():
                        ...
                        # Measure real mean luma (YAVG, 0-255) instead of the
                        # old PNG-file-size heuristic, which passed dark
                        # navy "black screen" frames (~55KB) without blinking.
                        luma = self._frame_mean_luma(frame_path)
                        visual_spotcheck["frame_mean_luma"].append(luma)
                        if luma is not None and luma < 12:
                            visual_spotcheck["black_frames_detected"] = True
                            visual_spotcheck["issues"].append(
                                f"Frame at {ts}s is effectively black "
                                f"(mean luma {luma:.1f}/255)"
                            )
                ...
                # These three checks require actually LOOKING at the frames —
                # they cannot be computed here. Flag them as unverified so a
                # "false" is never mistaken for "checked and passed". The
                # agent must read frame_paths with vision before presenting.
                visual_spotcheck["requires_agent_vision"] = [
                    "broken_overlays", "missing_assets", "unreadable_text"
                ]
            except Exception as e:
        ...
        if technical_probe.get("has_audio") and duration > 0:
            try:
                ...
                # Music can only be "present" if the edit actually declares a
                # music layer. The old heuristic (mean_vol > -50) reported
                # music_present=true on narration-only mixes — a false pass.
                ed_audio = (edit_decisions or {}).get("audio", {}) or {}
                music_expected = bool(
                    ed_audio.get("music") or (edit_decisions or {}).get("music")
                )
                audio_spotcheck["music_expected"] = music_expected
                ...
                if mean_vol is not None:
                    ...
                    if music_expected and mean_vol is not None and mean_vol <= -50:
                        audio_spotcheck["issues"].append(
                            "Music layer declared in edit_decisions but overall "
                            f"level is very low ({mean_vol:.1f} dB) — verify the "
                            "music actually made it into the mix"
                        )
