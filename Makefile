...
.PHONY: setup install install-dev install-gpu test test-contracts lint clean preflight demo demo-list hyperframes-doctor hyperframes-warm venv ensure-venv autoedit-ui-typecheck autoedit-dir autoedit-projects autoedit-dev cloud-render-offers cloud-render-reap cloud-render-queue cloud-render-live-test
...
test: ensure-venv
	$(RUN_PYTHON) -m pytest tests/ -v -m "not live"
...
# autoedit:    run the pipeline on one file      (INPUT=path PROMPT="...")
# autoedit-ui: job server + web UI (two terminals, or run both targets)
autoedit: ensure-venv
	$(RUN_PYTHON) -m lib.talking_head_edit.cli --input "$(INPUT)" --prompt "$(PROMPT)"

# autoedit-dir: every video in a folder as one build, best take picked per section
autoedit-dir: ensure-venv
	$(RUN_PYTHON) -m lib.talking_head_edit.cli --input-dir "$(DIR)" --prompt "$(PROMPT)"

autoedit-jobs: ensure-venv
	$(RUN_PYTHON) -m lib.talking_head_edit.cli --list

autoedit-projects: ensure-venv
...
autoedit-dev: ensure-venv
	$(RUN_PYTHON) run_autoedit_dev.py

autoedit-test: ensure-venv
	$(RUN_PYTHON) -m pytest tests/test_talking_head_*.py -q

# The root tsconfig includes only `src`, and Vite transpiles the UI with esbuild
# (types stripped, never checked) — so without this the UI has no type gate at all.
...
# cloud-render-reap:   orphan sweep; non-zero exit if it destroyed anything (see
#                      lib/cloud_render/reap.py). Run this first whenever a
#                      cloud rental is suspected to have leaked money.
# cloud-render-queue:  print the durable batch queue + flush_check thresholds.
cloud-render-offers: ensure-venv
	$(RUN_PYTHON) -m lib.talking_head_edit.cli --cloud-offers

cloud-render-reap: ensure-venv
	$(RUN_PYTHON) -m lib.cloud_render.reap

cloud-render-queue: ensure-venv
	$(RUN_PYTHON) -m lib.talking_head_edit.cli --cloud-queue-status

# cloud-render-live-test: rents a REAL Vast.ai instance and spends real money
# (~$0.01-0.05 total across the 3 smoke tests -- see tests/live/README.md).
# Refuses to run without both env vars explicitly set, so this can never fire
# from a script/CI that merely calls `make test`.
cloud-render-live-test: ensure-venv
	@if [ -z "$$VAST_LIVE_TEST" ] || [ -z "$$VAST_LIVE_MAX_USD" ]; then \
		echo "REFUSED: cloud-render-live-test rents a real Vast.ai instance and spends real money."; \
		echo "  Set both env vars to run it on purpose:"; \
		echo "    VAST_LIVE_TEST=1 VAST_LIVE_MAX_USD=0.05 make cloud-render-live-test"; \
		echo "  See tests/live/README.md for expected spend and what each test does."; \
		exit 1; \
	fi
	@echo "==> WARNING: about to rent a real Vast.ai instance. Ceiling: \$$$${VAST_LIVE_MAX_USD}."
	$(RUN_PYTHON) -m pytest tests/live/ -v -m live
