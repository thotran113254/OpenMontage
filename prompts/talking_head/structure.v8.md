...
Tôn trọng từng yêu cầu đúng phạm vi: không card → cards=[]; không cắt/giữ nguyên lời → cut_remove=[]; giữ khung gốc → không punch/shake; không chỉnh ảnh → grade={}. Nếu chỉ xin không mịn da thì không thêm skin_smooth/blemish_reduce, không tự bỏ card/cut hay các chỉnh màu khác đã được yêu cầu. Không thêm overlay, zoom, rung hay grade khách đã từ chối.

GỢI Ý KỸ THUẬT — mỗi lớp làm gì; chỉ dùng khi phù hợp yêu cầu và nội dung clip:
- Mặt người là mặc định. Overlay phải đáng để chiếm chỗ. Footage gốc đứng yên: không bịa camera shake, không zoom thở, không punch đều nhịp.
...
- Punch-in = **tuỳ chọn**, zoom nhẹ 1.04–1.08 lúc **một** đỉnh ý thật. Không bắt buộc. Không rải đều. Clip thường: 0–2 punch, cách >=3s, không trong card. CẤM punch nếu khách muốn giữ khung gốc.
- Shake/flash = **chỉ khi khách xin rõ** (rung, giật, flash, punch mạnh). Mặc định: không có event shake/flash.
...
- Cut: chỉ khi ghép joined nghe tự nhiên. Mức cắt theo luật ở mục 7.
- Endcard: recap 1 ý + CTA cụ thể từ lời nói. CẤM follow/đăng ký generic nếu lời không kêu.
- BGM/grade: **grade mặc định `{}`**. Chỉ điền khoá khi khách xin chỉnh màu/da/nét. saturation >1.03 làm da cam.
...
- {"type":"punch_in","atWord":i,"scale":1.06,"holdSec":1.1} — **không bắt buộc**. Nếu dùng: scale 1.04-1.08, mặt vẫn trong khung, không trong card, không dày hơn 1 punch / 3s.
- {"type":"sfx","atWord":i,"offsetSec":0,"name":"<tên file có thật>","volume":0.18} — offsetSec âm = kêu TRƯỚC từ. Nhẹ, không át giọng.
- {"type":"shake","atWord":i,"durSec":0.4,"intensity":6} và {"type":"flash","atWord":i,"durSec":0.15} — **CẤM** trừ khi yêu cầu riêng của khách nói rõ muốn rung/flash.{{broll_rule}}
...
6) "grade": {} 
- Mặc định object rỗng — giữ footage gốc, không tone, không mịn da, không nét thêm.
- CẤM tự thêm brightness/contrast/saturation/gamma/warmth/skin_smooth/blemish_reduce/sharpen "cho đẹp".
- Chỉ khi khách xin chỉnh ảnh mới điền các khoá cần thiết trong khoảng: brightness -0.05..0.08, contrast 1..1.1, saturation 1.0..1.03, gamma 1.0..1.06, warmth -5..10, skin_smooth 0..0.5, blemish_reduce 0..0.6, sharpen 0..1.0.
...
{{cut_rule}} Không cắt trùng cold_open.

{{topic_line}}{{style_notes}}{{source_rule}}
